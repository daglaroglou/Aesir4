...
";
    ...
    public class OdinMainWindow : Gtk.ApplicationWindow
    {
        ...
        private void InitializeFlashManagers()
        {
            try
            {
                // Only Odin4 is supported - no managers needed
                LogMessage("<OSM> Odin4 flash tool initialized");
            }
            catch (Exception ex)
            {
                LogMessage($"<OSM> ERROR: Failed to initialize: {ex.Message}");
            }
        }
    ...
    public class SettingsWindow : Gtk.Window
    {
        ...
        private void BuildUI()
        {
            ...
            // Thor and Heimdall removed - only Odin4 supported
